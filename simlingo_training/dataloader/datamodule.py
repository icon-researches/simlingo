...
def encode_uint8(strings: List[str], common_length: int) -> torch.Tensor:
    ...
    return torch.tensor(
        [bytearray(s, "utf-8") for s in padded_strings], dtype=torch.uint8
    )
...
class DataModule(LightningDataModule):
    def __init__(
        ...
        **cfg,
        ...
        self.num_image_tokens_per_patch = get_num_image_tokens_per_patch(
            self.encoder_variant
        )
        self.num_image_tokens_total = (
            self.num_image_tokens_per_patch * self.NUM_IMAGE_PATCHES
        )
    ...
    def setup(self, stage=None):
        if not self.predict:
            ...
            if self.driving_dataset is not None or self.dreamer_dataset is not None:
                ...
                for udd_i, (used_driving_dataset, used_train_partitions) in enumerate(
                    zip(used_driving_datasets, used_train_partitions)
                ):
                    num_datasets += 1
                    if used_train_partitions is not None:
                        ...
                        sample_weights_tmp = [
                            w / sum_sample_weights for w in sample_weights_tmp
                        ]
                        if (
                            self.driving_dataset is not None
                            and self.dreamer_dataset is not None
                        ):

                            if udd_i == 0:
                                sample_weights_tmp = [
                                    w * weights_driving for w in sample_weights_tmp
                                ]
                            else:
                                sample_weights_tmp = [
                                    w * weights_dreamer for w in sample_weights_tmp
                                ]
            ...
            if len(datasets) > 0:
                ...
                sample_weights = [
                    ...
                    if datasets[bucket].__len__() > 0
                ]
                bucket_list = [
                    bucket for bucket in bucket_list if datasets[bucket].__len__() > 0
                ]
                if len(bucket_list) != len(datasets):
                    # print in red
                    print(
                        f"\033[91mDatasets with 0 samples: {set(datasets.keys()) - set(bucket_list)}\033[00m"
                    )
                    print(f"\033[91mContinue without this bucket.\033[00m")
                datasets = {
                    key: value for key, value in datasets.items() if value.__len__() > 0
                }
                ...
                # === Fix: dataset weighted sampling ===
                self.train_dataset = torch.utils.data.ConcatDataset(
                    [datasets[bucket] for bucket in bucket_list]
                )

                bucket_sizes = [len(datasets[bucket]) for bucket in bucket_list]

                # Renormalize after empty buckets have been removed.
                bucket_probs = torch.tensor(sample_weights, dtype=torch.double)
                bucket_probs /= bucket_probs.sum()

                # WeightedRandomSampler consumes per-example weights.
                weights_train = torch.cat(
                    [
                        torch.full(
                            (size,),
                            prob.item() / size,
                        ...
                        for size, prob in zip(bucket_sizes, bucket_probs)
                    ]
                )

                # Preserve the original epoch-length policy.
                num_samples = int(
                    min(
                        size / prob.item()
                        for size, prob in zip(bucket_sizes, bucket_probs)
    ...
    def dl_collate_fn(self, data):
        ...
        ]  # we split the front forward into two patches (1x2)

        image_ff_pixel, image_ff_sizes = None, None
        image_ff_org = torch.tensor(
            np.asarray([data[i].image_ff_org_size for i in range(BS)])
        )
        ...
        for idx, img_to_consider in enumerate(self.IMAGES_TO_CONSIDER):
            ...
            images_batch_tensor = torch.tensor(
                np.asarray(
                    [
                        (
                            getattr(data[i], img_to_consider)
                            if getattr(data[i], img_to_consider) is not None
                            else np.zeros_like(img_tmp)
                        )
            ...
            if "internvl2" in self.encoder_variant.lower():
                ...
                images_processed = preprocess_image_batch(
                    ...
                    max_num_grid=grid_nums[idx],
                )
            else:
                raise ValueError(
                    f"Image preprocessing for {self.encoder_variant} not implemented"
                )
            ...
            images_pixel = images_pixel.view(
                BS, T, num_patches, C, new_height, new_width
            )
        ...
        answer_string_list = [
            data[i].answer[0]["content"][0]["text"] for i in range(BS)
        ]
        ...
        if self.base_dataset.use_1d_wps:
            waypoints = torch.tensor(
                np.asarray([data[i].waypoints_1d for i in range(len(data))])
            ).float()  # [B, F, 2] 11 future waypoints 0.2s apart
        else:
            waypoints = torch.tensor(
                np.asarray([data[i].waypoints for i in range(len(data))])
            ).float()  # [B, F, 2] 11 future waypoints 0.2s apart

        if self.predict:
            qa_templates = [
                data[i].qa_templates[0] if data[i].qa_templates is not None else None
                for i in range(BS)
            ]
            eval_infos = [
                data[i].eval_infos if data[i].eval_infos is not None else None
                for i in range(BS)
            ]
        else:
            qa_templates = None
        ...
        driving_input = DrivingInput(
            ...
            camera_intrinsics=torch.repeat_interleave(
                get_camera_intrinsics(W, H, 110).unsqueeze(0), BS, dim=0
            )
            .view(BS, 3, 3)
            .float(),
            camera_extrinsics=torch.repeat_interleave(
                get_camera_extrinsics().unsqueeze(0), BS, dim=0
            )
            .view(BS, 4, 4)
            .float(),
            vehicle_speed=torch.tensor(
                np.asarray([data[i].speed for i in range(len(data))])
            ).float(),  # [B, S] float32
            target_point=torch.tensor(
                np.asarray([data[i].target_points for i in range(len(data))])
            ).float(),  # [B, 2] float32
        ...
        driving_label = DrivingLabel(
            waypoints=waypoints,
            path=torch.tensor(
                np.asarray([data[i].path for i in range(len(data))])
            ).float(),  # [B, 3, RH, RW] uint8 [0, 255]
            answer=answer_label,
        ...
        return DrivingExample(
            ...
            run_id=encode_uint8(
                [data[i].measurement_path for i in range(BS)], 1000
            ),  # [B] str
